...
output_csv = "C:/Users/Azzla/Documents/covid_wa_daily_cases.csv"

oid = open(output_csv, 'w')
oid.write('Date, Daily Cases, 3 Day Moving Average, 5 Day Moving Average, 7 Day Moving Average\n')
...
i7 = 0
while j < num_days:
    days_before_today = num_days - (j)
    date = finddate(days_before_today, 'before')
    oid.write(str(date) + ', ')
    oid.write(str(wa_data[j]) + ', ')
    if j >= 3:
        i3 += 1
        oid.write(str(v3[i3]) + ', ')
    else:
